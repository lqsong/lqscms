...
switch ($step) {
    ...
    case 4:
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $datafile = $setting['add_test'] == 1 ? './inc/lqscms_data.sql' : './inc/lqscms.sql';
           ...
           // @file_get_contents('http://www.lqscms.com/api.php?c=Cms&a=getInstallInfo&email=' . base64_encode($setting['email']) . '&url=' . $_SERVER["HTTP_HOST"] . '&lang=' . $_SERVER['HTTP_ACCEPT_LANGUAGE']);

            $status = 'success_all';
            $info .= 'LQSCMS' . $lang['installation_successful'];
